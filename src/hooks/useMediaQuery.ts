...
const useMediaQuery = (query: string | TmediaQueries) => {
  const queryString = (Object.keys(mediaQueries) as Array<TmediaQueries>).includes(query as TmediaQueries)
    ? mediaQueries[query as TmediaQueries] :
    query;
  
  const [matches, setMatches] = useState<boolean>(() => window.matchMedia(queryString).matches);
